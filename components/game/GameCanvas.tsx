...
export default function GameTaskCanvas({
  ...
  useEffect(()=>{
    ...
    const drawIsland=()=>{
      ...
      r(W-170,8,160,28,'rgba(0,0,0,.5)');r(W-168,10,156,24,'#0a1020')
      tx(`SURVIVAL ${progress}/${Math.min(totalTasks,7)}`,W-90,28,'#00e5ff',10)
      r(W-168,10,Math.floor(156*(progress/Math.max(totalTasks,1))),4,'#00ff41')
      if(state==='idle') tx('▼ ANSWER TO SURVIVE',W/2-120,H-4,'#2a4a6a',5,'left')
      else if(state==='correct'){if(f>5){tx('✓ SURVIVED!',W/2,40,'#00ff41',9);if(f>15){tx('+XP',W/2,58,'#ffe600',7);if(f===16) burst(islandX+30,H-70,['#00ff41','#ffe600','#00e5ff'])}}}
    ...
    const drawCastle=()=>{
      ...
      ctx.fillStyle=bg;ctx.fillRect(0,0,W,H)
      stars(120);r(0,H-30,W,30,'#1a0a2a')
      const wallH=40+progress*12,cX=W-180
      ...
      if(progress>4){ctx.fillStyle='rgba(100,50,255,0.2)';ctx.beginPath();ctx.arc(cX+80,H-30-wallH-20,50,0,Math.PI*2);ctx.fill()}
      for(let i=0;i<3;i++){
        const ex=((f*2+i*90)%(W+60))-30,ey=H-50+Math.sin(ex*.02)*10
        r(ex-7,ey-14,14,12,'#cc0030');r(ex-3,ey-11,6,6,'#ff4466')
        ctx.fillStyle='#ff4466';ctx.font='9px monospace';ctx.textAlign='center';ctx.fillText('REQ',ex,ey-18)
      }
      const jump=state==='correct'?Math.abs(Math.sin(f*.3))*15:0
      drawChar(cX-30,H-40,'#aa44ff',false,jump)
      if(state==='correct'||state==='running'){
        ctx.fillStyle='rgba(100,50,255,0.4)';ctx.beginPath();ctx.arc(cX-30,H-50,25,0,Math.PI*2);ctx.fill()
      ...
      else if(state==='correct'){if(f>5){tx('✓ DEFENDED!',W/2,35,'#00ff41',9);if(f>15){tx('+XP',W/2,52,'#ffe600',7);if(f===16) burst(cX+80,H-80,['#aa44ff','#6644ff','#ffe600'])}}}
      else if(state==='wrong'){tx('✗ CASTLE HIT!',W/2,35,'#ff0040',7);r(cX,H-30-wallH,(f%15)*4,6,'#ff4400')}
    }
    ...
    const drawKingdom=()=>{
      ...
      ctx.fillStyle=bg;ctx.fillRect(0,0,W,H)
      stars(120);r(0,H-30,W,30,'#0a1520')
      const tables=[
      ...
      tables.forEach(tb=>{
        if(!tb.active){
          ctx.fillStyle='rgba(50,80,120,0.1)';ctx.fillRect(tb.x-20,H-90,40,50)
          ctx.strokeStyle='rgba(50,80,120,0.2)';ctx.lineWidth=1;ctx.setLineDash([2,3])
          ctx.strokeRect(tb.x-20,H-90,40,50);ctx.setLineDash([]);return
        }
        r(tb.x-20,H-90,40,50,tb.col);r(tb.x-20,H-92,40,6,'#3a5a7a');r(tb.x-20,H-96,40,6,'#4a6a8a')
        ctx.fillStyle='#88ccff';ctx.font='9px monospace';ctx.textAlign='center';ctx.fillText(tb.label,tb.x,H-98)
        for(let row=0;row<3;row++){
          r(tb.x-14,H-84+row*12,28,8,'rgba(0,0,0,.4)')
          ctx.fillStyle='#44aa77';ctx.font='9px monospace';ctx.textAlign='center';ctx.fillText(`ROW${row+1}`,tb.x,H-77+row*12)
        }
      })
      if(state==='running'||state==='correct'){
        ctx.fillStyle='rgba(0,229,255,.25)';const beamW=state==='running'?Math.min(f*12,480):480
        ctx.fillRect(40,H-60,beamW,9);tx('SELECT *',320,H-68,'#00e5ff',10)
      }
      const jump=state==='correct'?Math.abs(Math.sin(f*.3))*18:0
      drawChar(W/2-20,H-45,'#ffe600',state==='running',jump)
      r(W/2-26,H-68,12,6,'#ffcc00');r(W/2-23,H-73,6,6,'#ffe600')
      r(W-170,8,160,28,'rgba(0,0,0,.5)');r(W-168,10,156,24,'#0a1020')
      ...
      else if(state==='correct'){if(f>5){tx('✓ DATA FOUND!',W/2,35,'#00ff41',9);if(f>15){tx('+XP',W/2,52,'#ffe600',7);if(f===16) burst(W/2,H-50,['#4488ff','#44ffaa','#ffe600','#00e5ff'])}}}
    ...
    const drawTimeMachine=()=>{
      ...
      ctx.fillStyle=bg;ctx.fillRect(0,0,W,H)
      stars(130);r(0,H/2-2,W,4,'#1a2840')
      for(let i=0;i<Math.min(progress+1,totalTasks);i++){
        const cx=60+i*(W-120)/(Math.max(totalTasks-1,1))
        const active=i<progress
        r(cx-10,H/2-10,20,20,active?'#00ff41':'#1a2840');r(cx-8,H/2-8,16,16,active?'#00cc33':'#0e1828')
        ctx.fillStyle=active?'#00ff41':'#2a4a2a';ctx.font='9px monospace';ctx.textAlign='center';ctx.fillText(`v${i+1}`,cx,H/2+26)
        if(active&&i>0){ctx.fillStyle='rgba(0,255,65,.3)';ctx.fillRect(cx-10+(W-120)/(Math.max(totalTasks-1,1))-20,H/2-10,20,20)}
      }
      if(progress>2){
        ctx.strokeStyle='#ffe60066';ctx.lineWidth=2;ctx.setLineDash([4,4])
        const bx=60+2*(W-120)/(Math.max(totalTasks-1,1)),bx2=bx+(W-120)/(Math.max(totalTasks-1,1))*2
        ctx.beginPath();ctx.moveTo(bx,H/2);ctx.lineTo(bx+30,H/2-50);ctx.lineTo(bx2-30,H/2-50);ctx.lineTo(bx2,H/2)
        ctx.stroke();ctx.setLineDash([]);tx('feature',bx+(bx2-bx)/2,H/2-58,'#ffe600',9)
      }
      const capX=60+Math.min(progress,totalTasks-1)*(W-120)/(Math.max(totalTasks-1,1))
      r(capX-12,H/2-22,24,18,'#2a4a8a');r(capX-9,H/2-19,18,12,'#3a5a9a');r(capX-5,H/2-17,10,7,'rgba(0,229,255,.4)')
      const jump=state==='correct'?Math.abs(Math.sin(f*.3))*18:0
      ...
      r(20,8,160,28,'rgba(0,0,0,.5)');r(22,10,156,24,'#0a1020')
      ...
      else if(state==='correct'){if(f>5){tx('✓ COMMITTED!',W/2,35,'#00ff41',9);if(f>15){tx('+XP',W/2,52,'#ffe600',7);if(f===16) burst(capX,H/2-10,['#00ff41','#ffe600','#00e5ff'])}}}
    ...
    const drawMegaCity=()=>{
      ...
      ctx.fillStyle=bg;ctx.fillRect(0,0,W,H)
      stars(130);r(0,H-25,W,25,'#0a0a14')
      const systems=[
      ...
      if(state==='correct'&&f>15){
        ...
        systems.slice(0,progress).forEach((a,i)=>{
          if(i===0) return;const b=systems[i-1]
      ...
      r(W-175,8,168,28,'rgba(0,0,0,.5)');r(W-173,10,164,24,'#0a1020')
      ...
      else if(state==='correct'){if(f>5){tx('✓ SYSTEM ONLINE!',W/2,35,'#00ff41',8);if(f>15){tx('+XP',W/2,52,'#ffe600',7);if(f===16) burst(W/2,H/2,['#00ff41','#4488ff','#ffe600','#aa44ff'],20)}}}
      else if(state==='wrong') tx('✗ SYSTEM DOWN!',W/2,35,'#ff0040',8)
    }

    const games:Record<string,()=>void>={
      jump:drawJump, enemy:drawEnemy, evolution:drawEvolution,
      ...
      island:drawIsland, castle:drawCastle, kingdom:drawKingdom,
      timemachine:drawTimeMachine, megacity:drawMegaCity,
    }
  ...
  const GAME_LABELS: Record<string,{icon:string;name:string;col:string}> = {
    ...
    island:            {icon:'🏝',name:'ISLAND',          col:'#00e5ff'},
    castle:            {icon:'🏰',name:'SERVER CASTLE',   col:'#aa44ff'},
    kingdom:           {icon:'👑',name:'DATA KINGDOM',    col:'#4488ff'},
    timemachine:       {icon:'⏱',name:'TIME MACHINE',    col:'#ffe600'},
    megacity:          {icon:'🌆',name:'MEGA CITY',       col:'#00ff41'},
  }
