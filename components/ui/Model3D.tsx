...
function buildCastle(theme: ThemeId): THREE.Group {
  ...
  for (let i = -3; i <= 3; i++) {
    if (Math.abs(i) % 2 === 0) {
      for (const [bx, bz] of [[i*.4,1.4],[i*.4,-1.4],[1.4,i*.4],[-1.4,i*.4]]) {
        const b = new THREE.Mesh(new THREE.BoxGeometry(.28,.4,.28), mat); b.position.set(bx as number,0.5,bz as number); g.add(b)
      }
    }
  }
  for (const [tx, tz] of [[-1.2,-1.2],[1.2,-1.2],[-1.2,1.2],[1.2,1.2]]) {
    const twr = new THREE.Mesh(new THREE.CylinderGeometry(.42,.46,2,8), mat); twr.position.set(tx as number,.5,tz as number); g.add(twr)
    const roof = new THREE.Mesh(new THREE.ConeGeometry(.52,.9,8), mR); roof.position.set(tx as number,1.95,tz as number); g.add(roof)
    for (let j = 0; j < 8; j++) {
      const a = (j/8) * Math.PI * 2
      const bm = new THREE.Mesh(new THREE.BoxGeometry(.14,.22,.14), mat)
      bm.position.set((tx as number) + Math.cos(a)*.4, 1.55, (tz as number) + Math.sin(a)*.4); g.add(bm)
    }
  }
  ...
  for (const [wx, wy, wz] of [[.5,.4,1.4],[-.5,.4,1.4],[0,.3,-1.4],[1.4,.4,0],[-1.4,.4,0]]) {
    ...
    win.position.set(wx as number, wy as number, wz as number); g.add(win)
  }
  return g
}
...
export default function Model3D({ model, theme = 'golden', size = 200, rotate = true }: Props) {
