      ...
      <main style={{ flex:1, overflowY:'auto', display:'flex', flexDirection:'column', position:'relative' }}>
        ...
        <div style={{ display:'flex', alignItems:'stretch', borderBottom:'2px solid #0d1a28', background:'rgba(8,12,22,.96)', backdropFilter:'blur(20px)', flexShrink:0, position:'relative', zIndex:2, boxShadow:'0 2px 20px rgba(0,0,0,.5)' }}>
          ...
          <div style={{ padding:'16px 28px', borderRight:'1px solid #0d1a28', flexShrink:0, display:'flex', flexDirection:'column', justifyContent:'center', gap:6 }}>
            ...
            <div style={{ fontFamily:'var(--fp)', fontSize:11, color:'#00e5ff', letterSpacing:3, textShadow:'0 0 12px rgba(0,229,255,.4)' }}>INBOX</div>
          </div>
          ...
          <div style={{ flex:1, padding:'0 28px', display:'flex', alignItems:'center', gap:16 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              <span style={{ fontFamily:'var(--fp)', fontSize:11, color:'#00e5ff', letterSpacing:3, textShadow:'0 0 10px rgba(0,229,255,.3)' }}>{t('nt_title')}</span>
            </div>
            {unread > 0 && (
              <span style={{ fontFamily:'var(--fp)', fontSize:10, color:'#ff2d55', border:'1px solid #ff2d5533', padding:'3px 10px', background:'rgba(255,45,85,.06)', letterSpacing:1 }}>
                {unread} UNREAD
              </span>
            )}
            <div style={{ width:1, height:24, background:'rgba(13,20,38,.65)', backdropFilter:'blur(16px)' }}/>
            <span style={{ fontFamily:'var(--fp)', fontSize:10, color:'#2a3a54', letterSpacing:2 }}>{notifs.length} НИЙТ</span>
          </div>

          {unread > 0 && (
            <div style={{ padding:'0 28px', borderLeft:'1px solid #0d1a28', display:'flex', alignItems:'center', flexShrink:0 }}>
              <button onClick={markAll}
                style={{ fontFamily:'var(--fp)', fontSize:10, color:'#00e5ff', background:'transparent', border:'1px solid #00e5ff33', padding:'8px 18px', cursor:'pointer', letterSpacing:1, transition:'all .15s' }}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', borderBottom:'1px solid #0d1a28', flexShrink:0, background:'rgba(8,12,22,.96)', backdropFilter:'blur(20px)', zIndex:2 }}>
          {[
            ...
            <div key={s.k} style={{ padding:'16px 24px', borderRight: i < 2 ? '1px solid #0d1a28' : 'none', position:'relative' }}>
              ...
              <div style={{ fontFamily:'var(--fp)', fontSize:9, color:'#1a3050', letterSpacing:3, marginBottom:6 }}>{s.k}</div>
        ...
        <div style={{ flex:1, padding:'24px 40px', position:'relative', zIndex:2 }}>
          {notifs.length === 0 && (
            <div style={{ textAlign:'center', padding:'80px 0' }}>
              ...
              <div style={{ fontFamily:'var(--fp)', fontSize:9, color:'#1a3050', letterSpacing:3 }}>{t('nt_no_notifs')}</div>
            </div>
          )}
              ...
              return (
                <div key={n.id}
                  ...
                  <div style={{ flex:1, padding:'16px 18px', display:'flex', gap:14, alignItems:'flex-start' }}>
                    {/* type icon */}
                    <div style={{ width:34, height:34, flexShrink:0, background:`${tc.col}10`, border:`1px solid ${tc.col}33`, display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--fp)', fontSize:9, color:tc.col }}>
                      {tc.icon}
                    </div>

                    <div style={{ flex:1 }}>
                      <div style={{ fontFamily:'var(--fp)', fontSize:11, color: n.isRead ? '#3a5070' : '#c0d0e0', letterSpacing:1, marginBottom:5 }}>{n.title}</div>
                      <div style={{ fontFamily:'var(--fm)', fontSize:10, color:'#3a5070', lineHeight:1.6 }}>{n.message}</div>
                    </div>

                    <div style={{ flexShrink:0, display:'flex', flexDirection:'column', alignItems:'flex-end', gap:8 }}>
                      <div style={{ fontFamily:'var(--fp)', fontSize:9, color:'#1a3050', letterSpacing:1 }}>
                        {new Date(n.createdAt).toLocaleDateString('mn-MN')}
                      </div>
                      {!n.isRead && (
                        <button onClick={() => markRead(n.id)}
                          style={{ fontFamily:'var(--fp)', fontSize:9, color:tc.col, background:'transparent', border:`1px solid ${tc.col}33`, padding:'4px 10px', cursor:'pointer', letterSpacing:1, transition:'background .15s' }}
