        ...
        <div style={{ display:'flex', gap:14, alignItems:'flex-start' }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ display:'flex', alignItems:'flex-start', gap:8, marginBottom:5 }}>
              <span style={{ fontFamily:'var(--fp)', fontSize:9, color: assigned ? '#c0d0e0' : '#2a3a54', lineHeight:1.2 }}>{reward.title}</span>
            ...
            <div style={{ fontFamily:'var(--fm)', fontSize:7, color:'#3a5070', marginBottom:10, lineHeight:1.5 }}>{reward.description}</div>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              {reward.value > 0 && (
                <span style={{ fontFamily:'var(--fp)', fontSize:7, color:col, background:`${col}12`, padding:'2px 8px', border:`1px solid ${col}33` }}>
                  +{reward.value} {reward.type.toUpperCase()}
                </span>
              )}
              {claimed && userReward?.claimedAt && (
                <span style={{ fontFamily:'var(--fp)', fontSize:9, color:`${col}66`, marginLeft:'auto' }}>
                  ✓ {new Date(userReward.claimedAt).toLocaleDateString('mn-MN')}
                </span>
              )}
              {claimable && (
                <button onClick={() => onClaim(reward.id)} disabled={isClaiming}
                  style={{ fontFamily:'var(--fp)', fontSize:8, padding:'7px 18px', cursor: isClaiming ? 'wait' : 'pointer', background: isClaiming ? `${col}22` : `${col}22`, color:col, border:`1px solid ${col}`, marginLeft:'auto', transition:'background .15s,box-shadow .15s', boxShadow: !isClaiming ? `0 0 10px ${col}44` : 'none', letterSpacing:1 }}
              ...
              {!assigned && (
                <span style={{ fontFamily:'var(--fp)', fontSize:9, color:'#1a3050', marginLeft:'auto', letterSpacing:1 }}>LOCKED</span>
              )}
            </div>
          </div>
        </div>
      ...
      <main style={{ flex:1, overflowY:'auto', display:'flex', flexDirection:'column', position:'relative' }}>
        ...
        <div style={{ display:'flex', alignItems:'stretch', borderBottom:'2px solid #0d1a28', background:'rgba(8,12,22,.96)', backdropFilter:'blur(20px)', flexShrink:0, position:'relative', zIndex:2, boxShadow:'0 2px 20px rgba(0,0,0,.5)' }}>
          ...
          <div style={{ padding:'16px 28px', borderRight:'1px solid #0d1a28', flexShrink:0, display:'flex', flexDirection:'column', justifyContent:'center', gap:6 }}>
            <div style={{ fontFamily:'var(--fp)', fontSize:7, color:'#1a3050', letterSpacing:4 }}>ARENAHUB</div>
            <div style={{ fontFamily:'var(--fp)', fontSize:9, color:'#ff6b35', letterSpacing:3, textShadow:'0 0 12px rgba(255,107,53,.4)' }}>LOOT</div>
          </div>

          <div style={{ flex:1, padding:'0 28px', display:'flex', alignItems:'center', gap:16 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              <span style={{ fontFamily:'var(--fp)', fontSize:7, color:'#1a3050' }}>▸</span>
              <span style={{ fontFamily:'var(--fp)', fontSize:9, color:'#ff6b35', letterSpacing:3, textShadow:'0 0 10px rgba(255,107,53,.3)' }}>{t('rw_title')}</span>
            </div>
            {claimableRewards.length > 0 && (
              <span style={{ fontFamily:'var(--fp)', fontSize:8, color:'#ffd700', border:'1px solid #ffd70033', padding:'3px 10px', background:'rgba(255,215,0,.06)', letterSpacing:1, animation:'rw-pulse 2s ease-in-out infinite', '--pulse-col':'rgba(255,215,0,.2)' } as React.CSSProperties}>
          ...
          <div style={{ padding:'16px 28px', borderLeft:'1px solid #0d1a28', display:'flex', alignItems:'center', flexShrink:0 }}>
            <span style={{ fontFamily:'var(--fp)', fontSize:9, color:'#3a5070' }}>{user?.username?.toUpperCase()} · LV.{user?.level ?? 1}</span>
          </div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', borderBottom:'1px solid #0d1a28', flexShrink:0, background:'rgba(8,12,22,.96)', backdropFilter:'blur(20px)', zIndex:2 }}>
          {[
            ...
            <div key={s.k} style={{ padding:'18px 24px', borderRight: i < 3 ? '1px solid #0d1a28' : 'none', position:'relative' }}>
              ...
              <div style={{ fontFamily:'var(--fp)', fontSize:7, color:'#1a3050', letterSpacing:3, marginBottom:8 }}>{s.k}</div>
        ...
        <div style={{ flex:1, padding:'24px 40px', position:'relative', zIndex:2 }}>
          ...
          {/* Flash */}
          {flash && (
            <div style={{ fontFamily:'var(--fp)', fontSize:8, padding:'11px 18px', marginBottom:16, color: flash.ok ? '#00ff41' : '#ff2d55', border:`1px solid ${flash.ok ? '#00ff4144' : '#ff2d5544'}`, background:`${flash.ok ? '#00ff41' : '#ff2d55'}0d`, letterSpacing:1 }}>
              {flash.msg}
            </div>
          )}
          ...
          <div style={{ display:'flex', gap:6, marginBottom:18, flexWrap:'wrap', alignItems:'center' }}>
            {([['ALL','rw_all'],['AVAILABLE','rw_available'],['CLAIMED','rw_claimed'],['LOCKED','ach_locked']] as const).map(([f, tk]) => (
              <button key={f} onClick={() => setStatusFilter(f)} style={{ fontFamily:'var(--fp)', fontSize:8, padding:'7px 14px', cursor:'pointer', background: statusFilter===f ? 'rgba(255,107,53,.1)' : 'transparent', color: statusFilter===f ? '#ff6b35' : '#2a3a54', border:`1px solid ${statusFilter===f ? '#ff6b3566' : '#0d1a28'}`, letterSpacing:1, transition:'all .15s' }}>
            ...
            {types.map(tp => {
              ...
              return (
                <button key={tp} onClick={() => setTypeFilter(tp)} style={{ fontFamily:'var(--fp)', fontSize:8, padding:'7px 14px', cursor:'pointer', background: active ? `${col}12` : 'transparent', color: active ? col : '#2a3a54', border:`1px solid ${active ? col+'55' : '#0d1a28'}`, letterSpacing:1, transition:'all .15s' }}>
          ...
          {statusFilter === 'ALL' && claimableRewards.length > 0 && typeFilter === 'ALL' && (
            <div style={{ marginBottom:24 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12 }}>
                ...
                <span style={{ fontFamily:'var(--fp)', fontSize:8, color:'#ffd700', letterSpacing:2 }}>CLAIM ХИЙХ БОЛОМЖТОЙ</span>
          ...
          {/* Main grid */}
          {loading ? (
            <div style={{ fontFamily:'var(--fm)', fontSize:9, color:'#2a3a54', textAlign:'center', padding:'56px 0' }}>
            ...
            <div style={{ fontFamily:'var(--fm)', fontSize:9, color:'#2a3a54', textAlign:'center', padding:'48px 0' }}>Reward олдсонгүй</div>
          ) : (
            <>
              ...
              {statusFilter === 'ALL' && typeFilter === 'ALL' && [...claimedRewards, ...lockedRewards].length > 0 && (
                <>
                  <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12, marginTop: claimableRewards.length > 0 ? 12 : 0 }}>
                    ...
                    <span style={{ fontFamily:'var(--fp)', fontSize:8, color:'#2a3a54', letterSpacing:2 }}>ALL REWARDS</span>
          ...
          {!loading && allRewards.length === 0 && (
            <div style={{ textAlign:'center', padding:'56px 24px', border:'1px dashed #0d1a28', marginTop:8 }}>
              ...
              <div style={{ fontFamily:'var(--fp)', fontSize:7, color:'#1a3050', letterSpacing:2, marginBottom:8 }}>REWARD БАЙХГҮЙ</div>
              <div style={{ fontFamily:'var(--fm)', fontSize:7, color:'#0d1a28' }}>Admin panel → Rewards → ⚡ SEED DEFAULTS</div>
            </div>
          )}
        </div>
