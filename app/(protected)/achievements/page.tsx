      ...
      <div style={{ padding:'16px 16px 16px 20px', display:'flex', gap:14, alignItems:'flex-start' }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4 }}>
            <span style={{ fontFamily:'var(--fp)', fontSize:9, color: unlocked ? (rc?.col ?? '#ffd700') : '#2a3a54' }}>{ach.title}</span>
            {rc && (
              <span style={{ fontFamily:'var(--fp)', fontSize:9, color:rc.col, padding:'1px 5px', border:`1px solid ${rc.col}33`, marginLeft:'auto', flexShrink:0 }}>{rc.label}</span>
            )}
          </div>
          <div style={{ fontFamily:'var(--fm)', fontSize:7, color:'#3a5070', marginBottom:8, lineHeight:1.5 }}>{ach.description}</div>
          <div style={{ display:'flex', gap:8, alignItems:'center' }}>
            {ach.xpReward > 0 && <span style={{ fontFamily:'var(--fp)', fontSize:7, color:'#ffd700' }}>+{ach.xpReward} XP</span>}
            {ach.rewardAmount > 0 && <span style={{ fontFamily:'var(--fp)', fontSize:7, color:'#00e5ff' }}>+{ach.rewardAmount} {ach.rewardType}</span>}
            <span style={{ fontFamily:'var(--fp)', fontSize:9, color:'#1a3050', padding:'1px 5px', border:'1px solid #0d1a28', marginLeft:'auto' }}>
              {TYPE_LABEL[ach.type] ?? ach.type}
            </span>
          </div>
          {unlocked && unlockedAt && (
            <div style={{ fontFamily:'var(--fp)', fontSize:9, color:`${col}88`, marginTop:5 }}>
      ...
      <main style={{ flex:1, overflowY:'auto', display:'flex', flexDirection:'column', position:'relative' }}>
        ...
        <div style={{ display:'flex', alignItems:'stretch', borderBottom:'2px solid #0d1a28', background:'rgba(8,12,22,.96)', backdropFilter:'blur(20px)', flexShrink:0, position:'relative', zIndex:2, boxShadow:'0 2px 20px rgba(0,0,0,.5)' }}>
          ...
          <div style={{ padding:'16px 28px', borderRight:'1px solid #0d1a28', flexShrink:0, display:'flex', flexDirection:'column', justifyContent:'center', gap:6 }}>
            ...
            <div style={{ fontFamily:'var(--fp)', fontSize:9, color:'#ffd700', letterSpacing:3, textShadow:'0 0 12px rgba(255,215,0,.4)' }}>GAMIFY</div>
          </div>
          ...
          <div style={{ flex:1, padding:'0 28px', display:'flex', alignItems:'center', gap:16 }}>
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              <span style={{ fontFamily:'var(--fp)', fontSize:9, color:'#ffd700', letterSpacing:3, textShadow:'0 0 10px rgba(255,215,0,.3)' }}>{t('ach_title')}</span>
            </div>
            <div style={{ width:1, height:24, background:'rgba(13,20,38,.65)', backdropFilter:'blur(16px)' }}/>
            <span style={{ fontFamily:'var(--fp)', fontSize:8, color:'#2a3a54', letterSpacing:2 }}>{earned}/{total} ОЛГОСОН</span>
          </div>

          <div style={{ padding:'16px 28px', borderLeft:'1px solid #0d1a28', display:'flex', alignItems:'center', flexShrink:0 }}>
            <span style={{ fontFamily:'var(--fp)', fontSize:9, color:'#3a5070' }}>🏆 +{xpTotal} XP</span>
          </div>
        </div>
        ...
        <div style={{ flex:1, padding:'24px 40px', position:'relative', zIndex:2 }}>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:20, flexWrap:'wrap', alignItems:'center' }}>
            {([['ALL','ach_all'],['UNLOCKED','ach_unlocked'],['LOCKED','ach_locked']] as const).map(([f, tk]) => (
              <button key={f} onClick={() => setFilter(f)} style={{ fontFamily:'var(--fp)', fontSize:8, padding:'7px 16px', cursor:'pointer', background: filter===f ? 'rgba(255,215,0,.1)' : 'transparent', color: filter===f ? '#ffd700' : '#2a3a54', border:`1px solid ${filter===f ? '#ffd70066' : '#0d1a28'}`, letterSpacing:1, transition:'all .15s' }}>{t(tk)}</button>
            ))}
            ...
            {['ALL','EPIC','RARE','COMMON'].map(r => {
              ...
              return (
                <button key={r} onClick={() => setRarFilter(r)} style={{ fontFamily:'var(--fp)', fontSize:8, padding:'7px 14px', cursor:'pointer', background: active ? `${col}12` : 'transparent', color: active ? col : '#2a3a54', border:`1px solid ${active ? col + '55' : '#0d1a28'}`, letterSpacing:1, transition:'all .15s' }}>{r}</button>
              )
            })}
          </div>
          ...
          {/* Grid */}
          {loading ? (
            <div style={{ fontFamily:'var(--fm)', fontSize:9, color:'#2a3a54', textAlign:'center', padding:'48px 0' }}>{t('common_loading')}</div>
          ) : shown.length === 0 ? (
            <div style={{ fontFamily:'var(--fm)', fontSize:9, color:'#2a3a54', textAlign:'center', padding:'48px 0' }}>
